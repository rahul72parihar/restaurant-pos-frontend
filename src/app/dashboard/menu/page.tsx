...
export default function MenuPage() {
  ...
  const fetchData = () =>
    Promise.all([api.get("/menu/categories"), api.get("/menu/items")]).then(
      ([cats, its]) => {
        setCategories(cats);
        setItems(its);
        setFiltered(its);
  ...
  useEffect(() => {
    ...
    // Category filter
    if (activeCat !== "all") {
      f = f.filter((i: any) => i.category?.id === activeCat);
    }
    ...
    if (availability === "available") {
      f = f.filter((i: any) => i.isAvailable);
    }
    ...
    setFiltered(f);
  }, [activeCat, search, availability, items]);
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none px-4 pr-12 py-2.5 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 focus:outline-none focus:ring-2 focus:ring-orange-400 text-sm"
          >
          ...
          <div className="pointer-events-none absolute inset-y-0 right-4 flex items-center text-gray-400 text-[10px]">
            ▼
          </div>
        </div>
        <button
              ...
              : filtered.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${item.isVeg ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400" : "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400"}`}
                      >
                  ...
                  {[
                    ...
                    >
                      <input
                        type="radio"
                        value={v}
                        checked={form.isVeg === v}
                        onChange={(e) =>
                          setForm({ ...form, isVeg: e.target.value })
                        }
